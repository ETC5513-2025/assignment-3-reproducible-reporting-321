...
library(lubridate)
library(ggplot2)
case = read_csv("~/Desktop/5513/asgm3/COVID-19 confirmed case per million.csv")
death = read_csv("Total confirmed deaths due to COVID-19 per 100,000 people.csv")
vaccine = read_csv("COVID-19 vaccination.csv")
...
#| fig-cap: "Figure: COVID-19 confirmed case-death raees-vaccination by country)"
merge2 = left_join(merge1, death_long |>
                     select(Day, Country, Death_per_million), 
                   by = c("Day", "Country"))
merge2 |> 
...
In this project, we looked at COVID-19 confirmed cases, deaths, and vaccination rates in six countries: Canada, France, Germany, India, the UK, and the US.

We found three main things:

1. In most countries, case numbers went up before vaccines were given.

2. During big outbreaks, both cases and vaccine doses changed a lot from day to day.

3. A 7-day delay showed that when vaccination rates went up, new case numbers went down later on. This was most clear in places that started vaccines early and kept going.

This means that vaccines helped reduce the spread and saved lives. In the future, countries should act fast and start vaccinations early during health emergencies.
